...
const layout = async ({
  ...
  }: { data: { userByusername: IUser } } = await getProfile(username);
          ...
          <div defaultValue="posts" className="mt-4 bg-">
            <nav className="sticky md:top-12 top-24 bg-shade border-b z-10 shadow-sm px-4">
              <ul className="w-full max-w-5xl mx-auto flex gap-3 items-center justify-start h-full py-2">
                {[
                  { id: 1, name: "Posts", href: `/${username}` },
                  { id: 2, name: "About", href: `/${username}/about` },
                  { id: 3, name: "Friends", href: `/${username}/friends` },
                  { id: 4, name: "Photos", href: `/${username}/photos` },
                ].map((item) => (
                  <li key={item.id}>
